...
def geodesic_dist(r1, r2):
    """Minimal angular distance (rad) between two angle-axis vectors."""
    if r1.ndim == 1:
        R1 = Rotation.from_rotvec(r1).as_matrix()
        R2 = Rotation.from_rotvec(r2).as_matrix()
    else:
        R1 = Rotation.from_rotvec(r1.reshape(-1, 3)).as_matrix()
        R2 = Rotation.from_rotvec(r2.reshape(-1, 3)).as_matrix()
    cos_a = np.clip((np.trace(R1 @ R2.T) - 1) / 2, -1, 1)
    return np.arccos(cos_a)


def orient_diffs(arr):
    """Geodesic distance between consecutive orientation frames."""
    return np.array([geodesic_dist(arr[t], arr[t+1]) for t in range(len(arr)-1)])


def pose_diffs_arr(pose):
    """Per-frame mean geodesic pose change. pose: [T, 15, 3]"""
    return np.array([np.mean([geodesic_dist(pose[t, j], pose[t+1, j])
                               for j in range(pose.shape[1])]) for t in range(len(pose)-1)])
...
for h in range(B):
    ...
    o_diff = orient_diffs(ih['orient'][fids])
    if len(o_diff) > 0:
        ...
        print(f'  Orientation jumps: N/A (single frame)')
...
if B == 2:
    ...
    if common:
        ...
        print(f'\n─ Hand-hand distance: mean={hh_dist.mean():.2f}  min={hh_dist.min():.2f}  max={hh_dist.max():.2f} m  ({len(common)} shared frames)')
    else:
        print(f'\n─ Hand-hand distance: N/A (no shared frames)')
...
fig, axes = plt.subplots(B, 2, figsize=(18, 3.5 * B), squeeze=False, constrained_layout=True)
...
fig, axes = plt.subplots(B, 2, figsize=(20, 3.5 * B), squeeze=False, constrained_layout=True)
